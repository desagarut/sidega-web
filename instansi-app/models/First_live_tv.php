...
class First_live_tv extends CI_Model {
	...
	{
		$sql = "SELECT COUNT(id) AS id FROM live_tv WHERE enabled = 1 AND tipe='0'";
	...
	public function live_tv($offset=0, $limit=50)
	{
		...
		for ($i=0; $i<count($data); $i++)
		{
			...
			{
				$galeri = $data[$i]['id'];
				$sql = "SELECT link FROM live_tv WHERE ((enabled = '1') AND ((parrent = '".$galeri."') OR (id = '".$galeri."')) AND (link<>'')) LIMIT 1";
				$query = $this->db->query($sql);
				$row  = $query->row_array();
	...
	public function paging2($gal=0, $p=1)
	{
		...
		$sql = "SELECT COUNT(id) AS id FROM live_tv WHERE enabled = 1 AND (id = '$gal' or parrent = '$gal')";
	...
	{
		$sql = "SELECT * FROM live_tv WHERE id = '$parrent'";
		$query = $this->db->query($sql);
	...
	{
		$sql = "SELECT * FROM live_tv WHERE enabled = '1' and parrent = 0 order by rand() limit 4";
		$query = $this->db->query($sql);
		$data  = $query->result_array();
		return $data;
	}
