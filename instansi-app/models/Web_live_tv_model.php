<?php class Web_live_tv_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$this->urut_model = new Urut_Model('live_tv');
	}

	public function autocomplete()
	{
		return $this->autocomplete_str('nama', 'live_tv');
	}
	...
	public function insert()
	{
		...
		$outp = $this->db->insert('live_tv', $data);
	...
	public function update($id = 0)
	{
		...
		unset($data['old_link']);
		$outp = $this->db->where('id', $id)->update('live_tv', $data);
		if (!$outp) $_SESSION['success'] = -1;
	}
	...
	public function delete_gallery($id = '', $semua = false)
	{
		...
		$this->delete($id);
		$statsiun_tv = $this->db->select('id')->where('parrent', $id)->get('live_tv')->result_array();
		foreach ($statsiun_tv as $gallery) {
	...
	public function delete($id = '', $semua = false)
	{
		...
		$outp = $this->db->where('id', $id)->delete('live_tv');
	...
	{
		$image = $this->db->select('link')->get_where('live_tv', array('id' => $id))->row()->link;
	...
	{
		$sql = "SELECT * FROM live_tv WHERE id = ?";
	...
	{
		$sql = " FROM live_tv WHERE parrent = ? AND tipe = 2 ";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		return $sql;
	}
